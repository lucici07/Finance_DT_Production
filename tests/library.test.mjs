...
test('workspace folders retain links, new workspace keeps old tab, list export/import is local',async()=>{
...
const opened=context.waitForEvent('page');await page.getByRole('link',{name:'New workspace',exact:true}).click();const fresh=await opened;await fresh.locator('#workspaceLibrary').waitFor();
